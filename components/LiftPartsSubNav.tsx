...
export default function LiftPartsSubNav() {
  ...
  return (
    <div
      className={`hidden md:block fixed left-0 right-0 bg-white/95 backdrop-blur-md shadow-md transition-all duration-300 z-[9998] ${
        isSticky ? 'top-40 opacity-100' : 'top-0 opacity-0 pointer-events-none'
      }`}
    >
      <nav className="max-w-7xl mx-auto px-4 md:px-6 py-4">
        {/* Desktop horizontal navigation (mobile completely hidden) */}
